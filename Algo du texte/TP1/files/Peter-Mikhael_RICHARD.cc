...
int j=0,steps=0,occurences=0;
	for(int i=0;i<n;i++){
		while(j>=0){
			steps++;
			if(T[i+1]!=P[j+1]){
				cout<<"echec, T["<<i+1<<"]="<<T[i+1]<<" et P["<<j+1<<"]="<<P[j+1]<<endl;//log
				j=MP_next[j];
			}else{
				break;
			}
		...
		j++;
		if(j==m){
			occurences++;cout<<i-j+2<<endl;//log
			j=MP_next[j];
		}
	}
	cout<<"Table next :"<<endl;//log
	for(int it=0;it<m+1;it++){//log
	    cout<<MP_next[it]+1<<' ';//log
	}//log
	cout<<endl;//log
	cout<<"J'ai trouve "<<occurences<<" occurences de P dans T en "<<steps<<" etapes."<<endl;
delete [] MP_next;
...
int i=1, occurences=0, steps=0;
	while(i<n-m+2){
		...
		while(j>0){
			steps++;
			if(P[j]!=T[i+j-1]){cout<<"echec, T["<<i+j-1<<"]="<<T[i+j-1]<<" et P["<<j<<"]="<<P[j]<<endl;//log
				break;}
			j--;
		}
		if(j==0){
			occurences++;cout<<i<<endl;//log
			i+=D[1];
		}else{
			if(forte){
				...
				cout<<"Mvs char propose : "<<j-*it<<", bon suff propose : "<<D[j]<<endl;//log
				i+=max(D[j],j-*it);
			}else{
				cout<<"Mvs char propose : "<<j-R[T[i+j-1]].front()<<", bon suff propose : "<<D[j]<<endl;//log
