...
public:
	...
	vector<int> finMotif; // tableau indiquant les mots dont le noeud est la fin
	vector<Noeud*> vFils;
	Noeud* pere;
	Noeud* echec;

	// overload pour racine
	Noeud(){label = ' ';pere = NULL;echec = NULL;}
	// constructeur
	Noeud(char inchar,Noeud* sonpere){label = inchar;pere = sonpere;echec = NULL;}
	// destructeur
	~Noeud(){
	...
	Noeud* fils(char c){
		vector<Noeud*>::iterator it;
		for(it=vFils.begin();it!=vFils.end();it++){
			if(**it.label==c){return *it;}
		}
		return NULL;
	}
};
...
public:
	...
	void ajoutMot(string s,int indice){
		Noeud* noeudcourant = racine;
		Noeud* noeudsuivant;
		int i = 0;
		while(i<s.length()){
			noeudsuivant = noeudcourant.fils(s[i]);
			if(!noeudsuivant){
				while(i<s.length()){
					noeudsuivant(new Noeud(s[i],noeudcourant));
					noeudcourant.vFils.push_back(noeudsuivant);
		...
		noeudcourant.finMotif.push_back(indice);
	}

	Noeud* trouveMot(string s);

	void supprMot(string s){
		vector<Noeud*>* vFilsPere;
		Noeud* noeudcourant = racine;
		...
		if(!noeudcourant.finMotif.empty()){noeudcourant.finMotif.pop_front();}
		while(noeudcourant!=racine){
			if(noeudcourant.vFils.empty()){
				noeudsuivant=noeudcourant.pere;
				vFilsPere = &noeudsuivant.vFils;
				(*vFilsPere).erase(find((*vFilsPere).begin(),(*vFilsPere).end(),noeudcourant));
				delete(noeudcourant);
				noeudcourant=noeudsuivant;
			}else{return;}
		}
	...
	void calcule_erreur(){
		...
		for(it=noeudcourant.vFils.begin();it!=noeudcourant.vFils.end();it++){
			motcible = motcourant+**it.label;
			todo.push(pair<Noeud*,string>(*it,motcible));
			for(i=1;i<motcible.size();i++){
				noeudcible = trouveMot(motcible.substr(i,motcible.size()));
				if(noeudcible){break;}
			}
			if(noeudcible){
				for//TODO
				**it.echec = noeudcible;
			}else{**it.echec = racine;}
		}
		
	}

	int trouvePref(string s, int deb);
};
...
if(argc != 3){
	...
	cout<<"********************************************************************"<<endl;
	cout<<"Recherche d"<<((P.size()==1)?"u motif":"es motifs")<<" : "<<P.front();
